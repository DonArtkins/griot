...
**Phase P2** in `docs/planning/IMPLEMENTATION-ROADMAP.md`. Kit written (12 specs). **Not started; backend spec 09 prerequisite is met** (`GRIOT_SERVICE_TOKEN` plus `X-On-Behalf-Of`, HMAC `/api/webhooks/trigger`). Client implementations remain planned. Roadmap position: start after **P1 (web 01–09)** completes.
