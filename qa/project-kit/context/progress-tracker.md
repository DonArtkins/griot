...
**Phase P6** in `docs/planning/IMPLEMENTATION-ROADMAP.md` (Weeks 6–7). Kit written (14 specs). **Not started.** QA verifies the other six systems, so it runs last — but specs 01–03 are process docs with zero code dependencies (usable as gap fillers any time), and spec 04's base collection already exists (backend 08 ✅).
...
| 04 | API testing mastery (Postman) | Pending | backend 08 ✅ (base collection); backend 20 logging ✅; full folder 14 strict mode still needs backend 18/19/22 |
