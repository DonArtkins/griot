...
- k6 multi-tenant scenarios (qa 14): constant-arrival load across many organizations with per-org tags; p95 asserted **per tenant** within targets, not just aggregate.
- OWASP multi-tenant checks: IDOR across orgs (org B ids in URLs/bodies/GraphQL args), IDOT (token/org claim swap), forced browsing, suspended-org write attempts.
- Client-portal surface joins the axe runs (a11y per persona); OWASP review evidence lands in `docs/TENANT-ISOLATION-EVIDENCE.md` (qa 14).
