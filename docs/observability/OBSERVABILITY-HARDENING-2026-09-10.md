...
| **ai** | **PLANNED (spec 20):** once `ApiLoggingMiddleware`/`ApiLog` writers ship, every tool call is attributed via `ApiLogs.UserId` = the real OBO user id and the "audit rows per tool call" gate becomes testable. Per-role capability tiers arrive in backend 25 | Needs 20 (writers) + 25 (capability gateway) |
