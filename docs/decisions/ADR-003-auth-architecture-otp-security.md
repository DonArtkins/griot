...
| D12 | Email transport (Resend, not SMTP/raw SES) | Class: `ResendEmailService : IEmailService`. Constructor takes `HttpClient` (typed client configured with `ConfigurePrimaryHttpMessageHandler(() => new HttpClientHandler { AllowAutoRedirect = false })`) + `IConfiguration` + `ILogger` — SSRF redirects disabled; request URI restricted to `ApiUrl = "https://api.resend.com/emails"` fixed constant. `SendAsync` performs 3 `IConfiguration` lookups for the bearer token in this order: `Resend:ApiKey`, `RESEND_API_KEY`, `Resend__ApiKey` (double-underscore for Docker env). If all are null/whitespace, logs warning and returns `false`. Request: `POST /emails`; body `{ from, to: [email], subject, html, text: PlainText(html), tags: [{name:"system",value:"griot"},{name:"type",value:"auth"}] }`. From address resolution: `message.From` ?? Configuration["Resend:FromEmail"] ?? Configuration["RESEND_FROM_EMAIL"] → fallback `Griot <onboarding@resend.dev>` (Resend onboarding sender). Never throws: catch `Exception` (excluding `OperationCanceledException`) → `_logger.LogError(...)` + return `false`. Resend 429 Too Many Requests → false (no retry; manual retry via user re-clicking /otp/request). 502 from `/api/auth/otp/request` controller ONLY if Resend returns non-success; register NEVER fails on Resend error (best-effort OTP send after SQL commit). |
