...
| D12 | Email transport (Resend, not SMTP/raw SES) | Class: `ResendEmailService : IEmailService`. Constructor takes `IHttpClientFactory` typed client configured with `ConfigurePrimaryHttpMessageHandler(() => new HttpClientHandler { AllowAutoRedirect = false })` — SSRF redirects disabled; request URI restricted to `ApiUrl = "https://api.resend.com/emails"` fixed constant. Bearer token resolution chain (4 keys): `Environment.GetEnvironmentVariable("RESEND_API_KEY")` → Configuration["Resend:ApiKey"] → Configuration["Resend__ApiKey"]. Request: `POST /emails`; body `{ from, to: [email], subject, html, text: PlainTextStripper.Strip(html), tags: [{name:"system",value:"griot"},{name:"type",value:"auth"}] }`. From address resolution: Configuration["Resend:FromEmail"] / env `RESEND_FROM_EMAIL` → fallback `Griot <onboarding@resend.dev>` (Resend onboarding sender). Never throws: catch `HttpRequestException` + `Exception` → `_logger.LogWarning("Resend failed...")` + return `false`. Resend 429 Too Many Requests → false (no retry; manual retry via user re-clicking /otp/request). 502 from `/api/auth/otp/request` controller ONLY if Resend returns non-success; register NEVER fails on Resend error (best-effort OTP send after SQL commit). |
...
| G | **THIS ADR (Argon2id + 15m HS256 JWT ≥32-byte key validated at startup + 30d SHA256 at-rest opaque rotated refresh + FamilyId revoke + provider-default tx conditional ExecuteUpdate affected rows + Redis sliding limit + Resend OTP HMAC-peppered + branded template emails + Bearer-only no cookies + Resend client redirects disabled + admin inbox canonical fallback never skipped)** | Single .NET container; no extra services; memory-hard password hash; startup rejects short/missing JWT keys; zero-info-leak status codes for malformed tokens/logout; rotation window of theft = single use; family scope preserves honest second sessions; Redis singleton correct lifetime; OTP never plaintext; constant-time compares; SSRF redirects blocked on Resend HTTP client; admin notice always delivered (canonical fallback); Postman collection can exercise all codes; scalable to 2x backend replicas behind LB (Redis = global rate limit + shared multiplexer); CSRF-safe; OWASP A01/A02/A04/A05/A07/A10 largely closed; no auth-hardware cost for devs | More code than (A) or (D); Resend emails require env key for deliverability (local Postman tests can hit 502 without one); PasswordReset purpose scaffolded but no reset-send-link v1.1; TOTP future requires QR-code library NuGet. All tradeoffs acceptable for bootcamp MVP. | ✅ **ACCEPTED** |
