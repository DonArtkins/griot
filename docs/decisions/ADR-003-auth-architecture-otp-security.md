# ADR-003 — Griot Auth Architecture: JWT + Argon2 + Rotating Refresh Tokens + Family Revoke + Redis Rate Limiting + Resend OTP 2FA [own-stack]
...
3. **AI never writes to SQL (hard rule 2):** The `ai-agent` principal (Spec 09 future `GRIOT_SERVICE_TOKEN`) reads/writes ONLY through the .NET API — the auth stack must support a service-token claim mode WITHOUT a password login path. NOT YET implemented; ADR decision: reserve `sub=ai-agent:trigger-dev` + claim `griot.role=service` and reject such sub from `/login` route now.
...
5. **Dev UX zero-config local:** `dotnet run` from `backend/` must work for local dev with ONLY the git-ignored `appsettings.Local.json` for ConnectionStrings+JWT+Pepper; Redis auto-falls back to `localhost:6380` if config missing; Resend auto-falls back to `onboarding@resend.dev`; no hardcoded secrets.
...
- Bounce / complaint webhooks for Resend (§1.9 ops item; webhook controller scaffold → 501 currently).
...
| D2 | Access token | **JWT (JWS), HS256 symmetric** — 15-minute lifetime. Claims: `sub = Guid UserId.ToString("D")`, `email = user.Email`, `jti = crypto Guid` (opaque id; if future logout-by-jti is needed, Redis SET with TTL 15m; currently unused — jti present for audit). `iss = Configuration["JWT:Issuer"]`, `aud = Configuration["JWT:Audience"]`. Issuer signing key `JWT:Key` HS256 ≥ 32 bytes (256-bit). Refresh NOT embedded in access (small token, no DB on every request). No RS256 asymmetric — single backend service; key rotation via env redeploy. |
...
| D5 | Atomic rotation + concurrent refresh race | **RepeatableRead transaction + conditional UPDATE+INSERT order.** `RotateRefreshTokenAsync`: (1) open `BeginTransactionAsync(IsolationLevel.RepeatableRead)`; (2) `ExecuteUpdateAsync` WHERE `Id == toRevoke.Id && UserId == toRevoke.UserId && FamilyId == toRevoke.FamilyId && RevokedAt == null` SET `RevokedAt = now, ReplacedByTokenId = newTokenId, RevocationReason = "rotated"`; (3) if `affected != 1` → `Rollback` + `RevokeFamilyAsync(user, family)` + `return null` (reuse detected); (4) else INSERT new RefreshToken row with same FamilyId + new SHA256 TokenHash + ExpiresAt=now+30d + SaveChanges; (5) Commit. Winner = exactly 1 concurrent caller sees `affected == 1`; losers = 0 → rollback + family revoke. Two concurrent HTTP callers carrying the SAME refresh token cannot both succeed → the very act of racing = reuse evidence → family is revoked (defensive, but cannot tell which caller is legitimate without mutual TLS). |
...
| D10 | Email OTP 2FA architecture (research §1) | Three purposes: `email_verify`, `login_2fa`, `password_reset`. (1) Challenge creation `CreateOtpChallengeAndEmailAsync`: 6-digit code via `RandomNumberGenerator.GetInt32(0, 1_000_000).ToString("D6")` (NOT modulo-biased `Next`). Code hashed at rest with HMACSHA256(server `Otp:Pepper`, UTF8(code)) → `OtpChallenge.CodeHash` (NOT plain; pepper rotation on server wipes all stored codes = all active 2FA challenges expire; acceptable tradeoff 10-min window). `ExpiresAt = now + 10 min`, `AttemptCount = 0`, `Consumed = false`, `Purpose`, `RequestIp`, `CreatedAt`. Old challenges for same (email, purpose) NOT revoked on new create (overlapping windows OK; brute-force per-challenge). (2) Verify path: lookup latest active challenge, check `!Consumed && ExpiresAt>now && Purpose==request.Purpose`, HMAC hash incoming code SAME pepper, `CryptographicOperations.FixedTimeEquals` (constant time) compare hashes, `AttemptCount++`. If FAIL 5th attempt → `Consumed=true`, `LockedOut=true` in OtpVerifyResult (HTTP 429). If SUCCESS: `Consumed=true`. If `Purpose == "email_verify"`: `user.EmailVerified = true` + `SaveChanges`. Email sent via Resend with per-purpose branded template. Admin notice: when a user FIRST registers, `CreateOtpChallengeAndEmailAsync` ALSO renders `BrandedEmailTemplate.RenderNewAccountAdminEmail(userDto, createdUtc)` and sends to `Resend:ContactToEmail` (CONTACT_TO_EMAIL env fallback). |
| D11 | Email rendering | C# port of the Sababisha/Griot branded TypeScript email shell (bug.txt L~1200–1400). File: `BrandedEmailTemplate.cs`. Brand tokens match bug.txt TS constants: Bg=#EFEFEF, Card=#FFFFFF, Ink=#111111, Muted=#666666, Accent=#FF5A36, Border=#EEEEEE, Soft=#FAFAFA, FontSans=`font-family: Inter, ui-sans-serif, system-ui, -apple-system, sans-serif`, FontDisplay=`font-family: 'DM Serif Display', ui-serif, Georgia, serif`. No external images (deliverability risk — spam filters block image-only HTML). Logo = inline `<svg>` letter "G" in Accent color with a swervy underline stroke (replaces TS CID-embedded underline PNG). 3 OTP purposes use one method `RenderOtpEmail(purpose)` with switch: eyebrow/title/accent-word/preheader/body/footer different for each. Admin notice uses separate `RenderNewAccountAdminEmail` with key-value rows and detail section card. Text/plain body generated by stripping HTML tags (minimal fallback for plaintext MIME part; Resend supports `text` field alongside `html`). |
| D12 | Email transport (Resend, not SMTP/raw SES) | Class: `ResendEmailService : IEmailService`. Constructor takes `IHttpClientFactory` named client + `IOptions<ResendEmailOptions>`. Bearer token resolution chain (4 keys): `Environment.GetEnvironmentVariable("RESEND_API_KEY")` → `options.ApiKey` → Configuration["Resend:ApiKey"] → Configuration["Resend__ApiKey"] (double-underscore for Docker env). HttpClient `BaseAddress = new Uri("https://api.resend.com/")`. Request: `POST /emails`; body `{ from, to: [email], subject, html, text: PlainTextStripper.Strip(html), tags: [{name:"system",value:"griot"},{name:"type",value:"auth"}] }`. From address resolution: `options.FromEmail` / env `RESEND_FROM_EMAIL` → fallback `Griot <onboarding@resend.dev>` (Resend onboarding sender, per bug.txt requirement). Never throws: catch `HttpRequestException` + `Exception` → `_logger.LogWarning("Resend failed...")` + return `false`. Resend 429 Too Many Requests → false (no retry; manual retry via user re-clicking /otp/request). 502 from `/api/auth/otp/request` controller ONLY if Resend returns non-success; register NEVER fails on Resend error (best-effort OTP send after SQL commit). |
...
| D16 | Environment variable matrix (keys, fallbacks, secrets) | Contract in `docs/api/auth-contract.md` configuration table + `backend/appsettings.Local.json` (git-ignored dev). Canonical keys (Docker separator `__` supported): `ConnectionStrings__Default`, `JWT__Key`, `JWT__Issuer`, `JWT__Audience`, `Redis__Connection` (default `localhost:6380` fallback), `Otp__Pepper`, `Resend__ApiKey` / `RESEND_API_KEY`, `Resend__FromEmail` / `RESEND_FROM_EMAIL` (default `Griot <onboarding@resend.dev>`), `Resend__ContactToEmail` / `CONTACT_TO_EMAIL` (default `info.donartkins.ke@gmail.com` per bug.txt), `Cors__AllowedOrigins`. All values read via `IConfiguration`; zero hardcoded. |
...
| G | **THIS ADR (Argon2id + 15m HS256 JWT + 30d SHA256 at-rest opaque rotated refresh + FamilyId revoke + RepeatableRead tx conditional UPDATE + Redis sliding limit + Resend OTP HMAC-peppered + branded template emails + Bearer-only no cookies)** | Single .NET container; no extra services; memory-hard password hash; zero-info-leak status codes for malformed tokens/logout; rotation window of theft = single use; family scope preserves honest second sessions; Redis singleton correct lifetime; OTP never plaintext; constant-time compares; Postman collection can exercise all codes; scalable to 2x backend replicas behind LB (Redis = global rate limit + shared multiplexer); CSRF-safe; OWASP A01/A02/A04/A07 largely closed; no auth-hardware cost for devs | More code than (A) or (D); Resend emails require env key for deliverability (local Postman tests can hit 502 without one); PasswordReset purpose scaffolded but no reset-send-link v1.1; TOTP future requires QR-code library NuGet. All tradeoffs acceptable for bootcamp MVP. | ✅ **ACCEPTED** |
...
2. **`GRIOT_SERVICE_TOKEN` for Trigger.dev AI (Spec 09) is NOT implemented in this ADR.** Decision: reserved `sub` prefix `ai-agent:*` + `griot.role=service` claim type. Reject such sub in `/login` today so a future code path cannot accidentally authenticate service principals via password login.
3. **Resend `onboarding@resend.dev` deliverability:** Dev emails from onboarding@ often go to spam. For real users / production, Don must register a real sending domain in Resend dashboard, add SPF/DKIM/DMARC DNS records, update `Resend:FromEmail` → `noreply@griot.<tld>`. Tracked in research §1.8.
...
- **Spec 09 (AI service token):** Implement `GRIOT_SERVICE_TOKEN` issuer (symmetric HS256 key shared with Trigger.dev env); accept `Authorization: Bearer <griot_service_token>` via a dedicated scheme handler; map claim `griot.role=service` to `ai-agent` principal in EF with restricted repository methods.
- **Spec 11 (Blob storage):** Not related to auth.
- **Operational follow-up (any time):**
  1. Resend custom sending domain + SPF/DKIM/DMARC DNS records → update `Resend:FromEmail`.
  2. Add Resend bounce/complaint webhook handler at `POST /api/webhooks/resend` (currently scaffold 501). Mark consumed refresh / disabled OTP / user disabled on hard bounces.
