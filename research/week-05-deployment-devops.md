...
- **`ai/`** (Trigger.dev v3) → Trigger cloud only; scheduled + agent runs are durable and observable from the Trigger dashboard. Env: `GRIOT_SERVICE_TOKEN`, `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`, `GRIOT_API_URL`.
