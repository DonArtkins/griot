...
| F7 | P5 / P4 → P3 | GraphQL only, `GRIOT_SERVICE_TOKEN` → `ai-agent` principal (ReadWorkspace, CreateTask, UpdateTaskStatus, AddComment, CreateNotification — no deletes, no invites) |
...
F6 P1 → P5 (Trigger realtime WebSocket — Copilot stream + notification fan-out)
F7 P5/P4 → P3 (GraphQL, GRIOT_SERVICE_TOKEN → ai-agent principal)
...
ANNOTATION 1 (red callout attached to D1): "AI (P4 + P5) NEVER writes to SQL Server directly — every read/write goes through P3 (API) with GRIOT_SERVICE_TOKEN → ai-agent principal. No deletes, no invites, workspace-scoped."
