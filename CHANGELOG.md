...
### Backend 32 — Company onboarding & platform management (2026-09-12, `feature/backend/32-company-onboarding-lifecycle`)

- **Feature delivered:** SuperAdmin-only platform surface on `/api/organizations` — `POST /api/organizations` (onboard a company with a ONE-SQL-TRANSACTION seed: `Organizations` row + owner `OrganizationMembers` row `Invited` + the five spec-31 system roles via shared `RoleService` statics + default `Workspaces` row (`{slug}-default`) + `Onboarded` lifecycle event + AuditLogs row, serialized on the unique Slug key with the `RoleRepository` UPDLOCK/HOLDLOCK lock pattern) + best-effort branded Brevo owner invite (`RenderOrganizationInviteEmail`, 7-day plaintext two-Guid token — same convention as workspace invites); `GET /api/organizations` (paginated + name/slug search); `GET /api/organizations/{id}` (SuperAdmin or that company's Active Owner/Admin only); `POST …/{id}/suspend` + `/reactivate` (status flips + lifecycle/audit rows; the spec-29 `TenantGuard` gate turns suspended tenants' writes 403 `org_suspended` while reads/auth stay allowed); `POST …/{id}/transfer-ownership` (new owner must be a different Active member; previous owner demotes to Admin); `PUT …/{id}/plan` (metadata only); `POST /api/organizations/invites/{token}/accept` (invited, authenticated account — email match required; membership flips `Invited → Active` under an org-row-lock re-check so a concurrent accept conflicts).
- **Boundary:** platform paths run `IgnoreQueryFilters` + explicit organization targeting (the SuperAdmin session has no tenant scope; spec-29 filters fail closed without one); non-SuperAdmin mutations fail closed 403 on both `ITenantContext.IsSuperAdmin` (service) and the JWT `role` claim (controller). `ownerEmail` must already be a registered account (`Organization.OwnerId` is a non-nullable spec-29 FK). No forced refresh revocation on ownership transfer — claims re-resolve on the next refresh (spec 30 session resolution). One dead-code cleanup rode along: the duplicate post-rotation session resolution in `AuthService` was removed.
- **Tests:** 25 `OrganizationLifecycleServiceTests` (unit — seed payload, guards, transitions, accept) + 7 SQL-gated `OrganizationLifecycleSqlTests` (`Onboard_SeedCommitsAllRowsTogether`, `Onboard_MidSeedFailure_LeavesZeroPartialRows` — mid-seed unique-slug failure proves real rollback leaves zero rows, `Onboard_DuplicateSlug_SerializesToOneConflict`, `Suspend_ThenTenantWriteGate_FailsClosedWithOrgSuspended` + reactivate, `InviteAccept_EndToEnd_ActivatesOwner_SecondAccept_Conflicts`, `InviteAccept_EmailMismatch_Forbidden`, `Transfer_EndToEnd_RewritesOwnerAndDemotesPrevious`).
- **Verification:** `dotnet build` 0W/0E; spec-32 suites 25 + 7 green; full SQL-enabled suite run recorded in the backend progress tracker; spec 32 acceptance criteria all checked with named test evidence.
