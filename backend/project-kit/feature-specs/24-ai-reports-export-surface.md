...
Fetch every page beyond the 1,000-row cap with stable `(CreatedAt, Id)` ordering/cursors; aggregate only authorized rows. At job creation freeze a source manifest/revision and cutoff. Immutable event/evidence rows use that cutoff. Mutable task state is version-checked; a changed source invalidates/restarts the snapshot or produces an explicit incomplete result, never an unnoticed mixture of versions. Keep counts in typed aggregation code; model prose cannot alter them. Telemetry loss, retention limits and unclassified severities appear in coverage metadata.
