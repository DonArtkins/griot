...
| `test_run` | environment, executedAt UTC, tester from auth, scope/limitations, optional deployment ID, optional baseline test-run ID, rows `{caseId, module, scenario, result, severity?, defectTaskId?, evidenceRef?}`; result `pass/fail/blocked/not_run`; severity `critical/high/medium/low` applies only to findings; stable unique case IDs; no guessed outcomes |
