...
- Every transition writes `OrganizationLifecycleEvents` **and** `AuditLogs` (spec 20 pipeline) in the same transaction. Platform paths run `IgnoreQueryFilters` + explicit organization targeting (the SuperAdmin session has no tenant scope; spec-29 filters fail closed without one). Non-SuperAdmin mutations fail closed 403 on both `ITenantContext.IsSuperAdmin` (service) and the JWT `role` claim (controller, `AuthController` mirror).
