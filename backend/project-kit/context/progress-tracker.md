# Progress Tracker — Backend / API

## Current State

Week 2. The backend kit and 9 feature specs are written. The approved ERD is required before spec 02 (schema code).

| Spec | Title | Status |
|---|---|---|
| 01 | ERD & schema design (Figma/Figma Make) | Depends on PROMPTS/week-02 |
| 02 | SQL Server implementation (EF Core) | Pending ERD approval |
| 03 | Stored procedures & optimized queries | Pending |
| 04 | REST APIs (.NET 8) | Pending |
| 05 | GraphQL layer (HotChocolate) | Pending |
| 06 | Bulk operations & advanced data | Pending |
| 07 | API testing (Postman) | Pending |
| 08 | Auth: JWT + Argon2 + Redis [own-stack] | Pending |
| 09 | AI service token + webhooks [own-stack] | Pending |

## Next Steps

1. Approve the Figma Make ERD (figma-make-erd skill).
2. `dotnet new` solution scaffold per spec 02 Setup.
3. Implement specs sequentially.

## Session Notes

- **2026-09-03** — Kit restructured to one spec per bootcamp deliverable; 9 specs created.
