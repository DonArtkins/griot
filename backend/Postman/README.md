...
| Symptom | Fix |
|---|---|
| Register returns `500` "ConnectionStrings:Default is not configured" | Set `SABABISHA_SA_PASSWORD` in your environment (or `.env`), then either start the API with the env var (`ConnectionStrings__Default` with the expanded password) or write the expanded value into the git-ignored `backend/src/Griot.Api/appsettings.Local.json` → `ConnectionStrings:Default` = `Server=localhost,14333;Database=Griot;User Id=sa;Password=<actual password>;TrustServerCertificate=True`. |
