# Diagrams — Figma / Figma Make Reference Artifacts

Everything generated in Figma / Figma Make that governs or documents Griot's implementation. **Code agents read these before writing code** (per `ai-workflow-rules.md`).

## Folders

| Folder | Holds | Governs |
|---|---|---|
| `erd/` | Entity-relationship diagrams (Week 2 deliverable) | backend feature 02/feature 03 schema; every entity/enum name is a contract here |
...
|---|---|---|---|---|---|
| *(pending)* `griot-erd-v1.0.0.png` | v1.0.0 | Week 2 | Figma Make (`PROMPTS/week-02/01-database-schema-erd-figma-make.md`) | Awaiting generation | 13 tables + 4 enums; contract for backend feature 02/feature 03 |

## Rules

1. Only **approved** diagrams enter here (approval = human sign-off in Figma Make + this ledger updated).
2. A schema change means: update the ERD first, re-approve, **then** update backend feature 02/feature 03 + context + progress-tracker (contracts sync gate).
