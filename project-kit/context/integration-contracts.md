...
| backend (PLANNED) | `SUPERADMIN__EMAIL` (+ `SUPERADMIN__PASSWORD` first-boot optional) | SuperAdmin bootstrap on startup (multi-tenant spec 30). `PlatformRole` upgraded idempotently + audit-logged. Password used ONLY on first-boot if no SuperAdmin row exists; subsequent boots ignore. |
