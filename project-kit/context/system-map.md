...
| ai | backend | REST + GraphQL (service-token OBO; GraphQL is the primary read path) | `GRIOT_SERVICE_TOKEN` + `X-On-Behalf-Of` (real-user OBO principal — 4 scopes today; +`CreateReport` PLANNED spec 24; role-tiered capability filtering PLANNED spec 25; no deletes/invites) |
| mcp | backend | REST + GraphQL (service-token OBO) | `GRIOT_SERVICE_TOKEN` + `X-On-Behalf-Of` (real-user OBO; role-aware capability gate per spec 25) |
